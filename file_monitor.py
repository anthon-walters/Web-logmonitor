...
class FileMonitor:
    ...
    def __init__(self):
        ...
        # Try to connect to the base path
        try:
            if os.path.exists(self.base_path):
                self.connected = True
                self.logger.info(f"Successfully connected to path: {self.base_path}")
            else:
                self.logger.error(f"Cannot access path: {self.base_path}")
        except Exception as e:
            self.logger.error(f"Error connecting to path: {str(e)}")
